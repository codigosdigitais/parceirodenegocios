<?php
class adicionaisc_model extends CI_Model {
    ...
    function get($table,$fields,$where='',$perpage=0,$start=0,$one=false,$array='array'){
		...
		$sql = "
				SELECT 
					p.*, 
					f.idCliente, 
					f.razaosocial AS nomeFuncionario, 
				...
				FROM
					...
					parametro AS pa 
				WHERE 
					p.idAdministrativo = f.idCliente
				AND
					f.idEmpresa = ".$idEmpresa."
				AND 
					pa.idParametro = p.idParametro 
				AND p.modulo = 'cliente'
	...
	public function getBase($tabela, $cod, $order, $idAtual = null)
	{
		...
		if ($tabela == 'cliente') {
			...
			if ($idAtual != null) {
				$this->db->or_where('idCliente = (select idAdministrativo from adicional where idAdicional = '.$idAtual.')');
			}
		}
		...
		if ($tabela == 'fornecedor') {
			$this->db->where('situacao', 1);
			if ($idAtual != null) {
				$this->db->or_where('idFornecedor IN (select idAdministrativo from adicional where idAdicional = '.$idAtual.')');
			}
		}
