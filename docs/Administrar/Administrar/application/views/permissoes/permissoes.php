...
<table class="table table-bordered ">
    ...
    <tbody>
        <?php foreach ($results as $r) {
            if($r->situacao == 1){$situacaoPerfil = 'Ativo';}else{$situacaoPerfil = 'Inativo';}
            echo '<tr>';
            ...
            echo '<td>
                      <a href="'.base_url().'permissoes/editar/'.$r->idPermissao.'" class="btn btn-info " title="Editar Permissão"><i class="icon-pencil icon-white"></i></a>
                      <a href="#modal-excluir" role="button" data-toggle="modal" permissao="'.$r->idPermissao.'" class="btn btn-danger " title="Desativar Permissão"><i class="icon-remove icon-white"></i></a>
                  </td>';
            echo '</tr>';
        }?>
        <tr>
