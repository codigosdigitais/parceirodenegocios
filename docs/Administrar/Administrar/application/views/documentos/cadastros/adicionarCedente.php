...
<?
	#Define Títulos Topo
	if($this->router->method=="cedenteEditar")
	{
		$tituloBase = "Editar Documentação de Cedente";
	} 
	else
	{
		$tituloBase = "Cadastro de Documentação para Cedentes"; 
	}
?>
               ...
               ?>
                ...
                <fieldset>
						...
						<div class="line">
							<p>
							  ...
							  <select class="input-xxlarge" name="idAdministrador" id="idAdministrador" style="width: 715px !important;" autofocus>
							    ...
							    <option value="<? echo $listaCedente->idCedente; ?>" <? if(isset($result[0]->idAdministrador)){ if($listaCedente->idCedente==$result[0]->idAdministrador){ echo "selected"; } } ?>><? echo $listaCedente->razaosocial; ?></option>
							    <? } ?>
						      </select>
							  <br>
						  </p>
					...
					<div class="button-form line">										    
                        ...
                        <? } ?>
                        <a href="<?php echo base_url() ?>documentos/cedente" class="btn"><i class="icon-arrow-left"></i> Voltar</a>
                        </div>
				    </div>
		    	</fieldset>
	    	</form>
