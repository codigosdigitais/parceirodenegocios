...
<?
	#Define Títulos Topo
	if($this->router->method=="contratoEditar")
	{
		$tituloBase = "Editar Documentação de Contrato";
	} 
	else
	{
		$tituloBase = "Cadastro de Documentação para Contratos"; 
	}
?>
               ...
               ?>
                ...
                <fieldset>
						...
						<div class="line">
							<p>
							  ...
							  <select class="input-xxlarge" name="idAdministrador" id="idAdministrador" style="width: 715px !important;" autofocus>
							    ...
							    <? foreach($this->data['listaContrato'] as $listaContrato){ ?>
							    <option value="<? echo $listaContrato->idContrato; ?>" <? if(isset($result[0]->idAdministrador)){ if($listaContrato->idContrato==$result[0]->idAdministrador){ echo "selected"; } } ?>><? echo $listaContrato->RazaoSocialContrato; ?></option>
							    <? } ?>
						      </select>
							  <br>
						  </p>
					...
					<div class="button-form line">										    
                        ...
                        <? } ?>
                        <a href="<?php echo base_url() ?>documentos/contrato" class="btn"><i class="icon-arrow-left"></i> Voltar</a>
                        </div>
				    </div>
		    	</fieldset>
	    	</form>
