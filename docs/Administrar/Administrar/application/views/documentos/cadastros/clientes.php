<?php if($this->permission->canInsert()){ ?>
    <a href="<?php echo base_url();?>documentos/clienteAdicionar" class="btn btn-success"><i class="icon-plus icon-white"></i> Adicionar Documento</a>    
<?php } ?>
    ...
    <tbody>
        <?php foreach ($results as $r) {
            ...
            if($this->permission->canUpdate()){
                echo '<a href="'.base_url().'documentos/clienteEditar/'.$r->idDocumento.'" style="margin-right: 1%" class="btn btn-info " title="Editar Documento"><i class="icon-pencil icon-white"></i></a>'; 
            }
...
<div id="modal-excluir" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <form action="<?php echo base_url() ?>documentos/clienteExcluir" method="post" >
