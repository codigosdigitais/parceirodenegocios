...
<?php if($this->permission->canInsert()){ ?>
    <a href="<?php echo base_url();?>descontos" class="btn btn-success" style="width: 135px;"><i class="icon-plus icon-white"></i> Adicionar Desconto</a>
...
if(!$results){?>
        ...
        <div class="widget-content nopadding">
            <table class="table table-bordered">
                ...
                <tbody>
                    <tr>
                        <td colspan="6">Nenhum Descontos Cadastrado</td>
                    </tr>
                </tbody>
            </table>
        </div>
...
<div id="Pesquisar" <? if(isset($_GET['razaosocial']) || isset($_GET['cnpj'])){ ?> <? } else { ?> style="display:block"<? } ?>>
		    	<form class="form-inline" method="post" action="<?php echo base_url('descontos/pesquisar'); ?>">
                <fieldset>
						...
						<div class="line">
							...
							<select class="input-xxlarge" name="idTipo" id="idTipo" style="width: 715px !important;">
								...
								<?
                                    foreach($this->data['parametroDesconto'] as $parametroDesconto){ 
                                ?>
                                <option value="<?=$parametroDesconto->idParametro;?>" <? if($this->input->post('idTipo')!=NULL){ if($parametroDesconto->idParametro==$this->input->post('idTipo')){ echo "selected"; } } ?>><?=$parametroDesconto->parametro;?></option>
                                <? } ?>								
							</select>
						</div>
...
<table class="table table-bordered ">
    ...
    <tbody>
		...
		foreach ($results as $r) {
            echo '<tr>';
			echo '<td><center>'.$r->idDesconto.'</center></td>';
            ...
            if($this->permission->canSelect()){
                echo '<a href="'.base_url().'descontos/visualizar/'.$r->idDesconto.'" style="margin-right: 1%" class="btn" title="Ver mais detalhes"><i class="icon-eye-open"></i></a>'; 
            }
            ...
            if($this->permission->canDelete()){
                echo '<a href="#modal-excluir" role="button" data-toggle="modal" Desconto="'.$r->idDesconto.'" style="margin-right: 1%" class="btn btn-danger" title="Excluir Desconto"><i class="icon-trash icon-white"></i></a>'; 
            }
