<?php
class proventos_model extends CI_Model {
	...
	function getPesquisar($limit){
		$idEmpresa = $this->session->userdata['idEmpresa'];
		
		if(empty($start) || !is_numeric($start)) $start = 0;
	
		$this->db->select("provento.*, funcionario.idFuncionario, funcionario.nome as nomeFuncionario, parametro.parametro as nomeParametro,
					(CASE provento.referencia 
						when 1 then 'Janeiro'
		...
		if($this->input->post('idFuncionario')){ $this->db->where('provento.idFuncionario', $this->input->post('idFuncionario')); }
		if($this->input->post('idTipo')){ $this->db->where('provento.tipo', $this->input->post('idTipo')); }
		if($this->input->post('periodo')){ $this->db->where('provento.referencia', $this->input->post('periodo')); }
		if($this->input->post('data_inicial') and $this->input->post('data_final')){ $this->db->where("provento.data BETWEEN '".$this->input->post('data_inicial')."' AND '".$this->input->post('data_final')."'"); }
		
		$this->db->where('funcionario.idEmpresa', $idEmpresa);
		
		$this->db->join("funcionario", "funcionario.idFuncionario=provento.idFuncionario", "LEFT");		
		$this->db->join("parametro", "parametro.idParametro=provento.tipo", "LEFT");
		$this->db->limit($limit);
		
		$consulta = $this->db->get('provento')->result();
		return $consulta;
	}
    ...
    function getById($id){
        ...
        $this->db->where('idProvento',$id);
        $this->db->limit(1);
        return $this->db->get('provento')->row();
    ...
    function count() {
    	...
    	$this->db->join('funcionario b', 'a.idFuncionario = b.idFuncionario', 'RIGHT');
        $this->db->where('b.idEmpresa', $idEmpresa);
        $this->db->get('provento a');
        //die($this->db->last_query());
	...
	public function getBase($tabela, $cod, $order, $idAtual = null)
	{
		...
		if ($tabela == 'funcionario') {
			$this->db->where('situacao', 1);
			if ($idAtual != null) {
				$this->db->or_where('idFuncionario IN (select idFuncionario from provento where idProvento = '.$idAtual.')');
			}
		}
