		...
		foreach($consulta as &$valor){
			...
			$sql = "
						...
						AND
							...
							cs.idChamada = '".$valor->idChamada."'
						ORDER BY idChamadaServico ASC

			";

			$valor->chamadaServico = $this->db->query($sql)->result();
			...
			$sqlSolicitante = "
							...
							WHERE 
								...
								idClienteResponsavel = '".$valor->solicitante."' 

								

			";

			$valor->chamadaResponsavel = $this->db->query($sqlSolicitante)->row('nome');
